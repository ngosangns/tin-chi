...
const jsonPath =
  import.meta.env.VITE_REMOTE_JSON_FILE ?? './tinchi.json' + `?timestamp=${new Date().getTime()}`
const excelPath =
  import.meta.env.VITE_REMOTE_EXCEL_FILE ?? './tinchi.xlsx' + `?timestamp=${new Date().getTime()}`
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((result: any) => {
      ...
      if (isOpenModel.value)
        setTimeout(() => (triggerRerenderTable.value = !triggerRerenderTable.value), 150)
    })
    ...
    <div v-else>
      ...
      <label id="my-modal-4-content" for="my-modal-4" class="modal cursor-pointer">
        <label class="modal-box w-9/12 max-w-3xl relative rounded-lg" for="">
          <label for="my-modal-4" class="btn btn-sm btn-circle sticky top-2 left-full z-10"
            >✕</label
          >
          <div class="w-full">
            <div class="mb-8 w-full mx-auto">
              ...
              <ul class="list-disc ml-4 mb-2" style="padding-left: 0.125rem">
                <li>
                  Check vào nút đăng ký các môn muốn học, sau đó chọn lớp cho từng môn, tiết nào bị
                  trùng thì sẽ chuyển sang màu đỏ, bấm vào ô trong lịch để xem những môn của tiết
                  đấy.
                </li>
                <li>
            ...
            <template
              ...
              v-for="[majorName, major] in Object.entries(
                        ...
                        <tr>
                          ...
                          <td class="w-40">
                            <select
                              ...
                              @change="
                                (e) => onChangeSelectSubjectClass(majorName, subject, e.target)
                              "
                            >
                ...
                <tr>
                  <td colspan="2" class="text-xs text-center">
                    <span class="font-bold text-lg">
                      {{ dayOfWeekMap[new Date(date).getDay()] }} </span
                    ><br />
                      ...
                      <template v-if="calendarTableContent[date][session].length">
                        <div
                          ...
                          :class="{
                            ...
                            'dropdown-top': dateIndex === calendar.dateList.length - 1
                          }"
                        >
                          <button
                            tabindex="0"
                            ...
                            :class="{
                              'btn-accent': calendarTableContent[date][session].length > 1
                            }"
                          >
                          ...
                          <ul
                            ...
                            :class="{
                              'bg-primary': calendarTableContent[date][session].length === 1,
                              'bg-accent-focus': calendarTableContent[date][session].length > 1
                            }"
                          >
                            <template
                            ...
                            >
                              <li
                                class="text-lg"
                                :class="{
                                  'text-base-100': calendarTableContent[date][session].length > 1
                                }"
                              >
                                {{
                                  calendarTableContent[date][session].length === 1 ? '' : '-'
